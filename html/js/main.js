...
$(document).ready(function(){ 	
	...
	if(domain === '') {
		...
		if(absoluteUrl === 0) {
			...
			var newDomain = window.location.pathname.split('/')[1];
	...
	$(function () {
		...
		for ( var i = 0, j = pages.length; i < j; i++ ) {
		    ...
		    if($.cookie("currentPage") == pages[i]) {
				...
				cBtn.children('a').attr('title','显示导航栏');
	...
	$( '.collapseBtn' ).bind( 'click', function(){
		...
		if ($this.hasClass('leftbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				$this.children('a').attr('title','显示导航栏');
		...
		if ($this.hasClass('rightbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				$this.removeClass('hide');
				$('.collapseBtn.rightbar').css('top', '70'+'px').css('right', '18'+'px').removeClass('shadow');
